...
byte PPU::readPPU(uint16_t address, Mapper *mapper) {
    ...
    if (address < 0x2000) {
        ...
        std::cerr << "ERROR: PPU encountered unrecognized read (address 0x" << std::hex << address << std::dec << ')'
                  << std::endl;
    }
    return 0;
}
            ...
            for (int i = 0; i < 64; ++i) {
                ...
                if (count < 8) {
                    ...
                    if (this->flag_sprite_size == 0) {
                        ...
                        address = (static_cast<uint16_t>(table) << 12) + (static_cast<uint16_t>(tile) << 4) +
...
void PPU::writePPU(uint16_t address, byte value, Mapper *mapper) {
    ...
    if (address < 0x2000) {
        ...
        std::cerr << "ERROR: PPU encountered unrecognized write (address 0x" << std::hex << address << std::dec << ')'
                  << std::endl;
    }
}
...
void PPU::writeRegisterPPU(uint16_t address, byte value, Mapper *mapper, CPU *cpu) {
    ...
    switch (address) {
        ...
        case 0x4014:
            ...
            for (int i = 0; i < 256; ++i) {
                this->oam_tbl[this->oam_addr] = cpu->readb(address);
                ++this->oam_addr;
                ++address;
            }
            cpu->stall += 513;
